...
export class WorkbookService {
            ...
            datasets: [
                {
                    ...
                    data: this.months().map((month) => month.rows.reduce((sum, row) => {
                        const day = this.fx.dayIndexForRow(month, row);
    ...
    private restoreSettings(): void {
        ...
        try {
            ...
            const yearHint = typeof parsed.calendarYear === 'number'
                ? parsed.calendarYear
                : this.fx.calendarYear();
            if (Array.isArray(parsed.fx)) {
                this.fx.load(parsed.fx, yearHint);
            }
            if (typeof parsed.displayCurrency === 'string') {
                this.fx.setDisplayCurrency(parsed.displayCurrency);
            }
            if (typeof parsed.calendarYear === 'number') {
                this.fx.setCalendarYear(parsed.calendarYear);
            }
        } catch {
            return;
        }
    ...
    static splitMeta(csvData: string): {
        meta: {
            ...
            opening: Record<string, number>;
            constants: ConstantDef[];
            fx: {year?: number; currencies?: unknown; display?: string} | null;
            settings: {
                saldoColumnPrefs?: Record<string, SaldoColumnPref>;
                saldoTotalPref?: SaldoColumnPref;
                saldoColumnsOpen?: boolean;
